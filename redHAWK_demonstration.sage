...
class SignatureScheme:
    ...
    def DecompressSK(self, skc,pk):
        """
        Decompress a HAWK secret key by recomputing F and G using the public key
...
def test_run(n=32, sigma_kg=1.5, sigma_sig=1, sigma_ver=1.1):
    ...
    sk_ = Sig.DecompressSK(Sig.CompressSK(sk),pk)
    if sk_ != sk:
...
Ns = [2^i for i in range(4,10)]
num_test = 10
Ts = []
for n in Ns:
   print("\ntesting for degree", n)
   tot = 0
   for _ in num_test *" ":
      t0 = time.time()
      test_run(n)
